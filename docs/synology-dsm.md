...
```bash
docker run --rm ghcr.io/bursula/bursula:0.3.13 deployment export | tar -x && ./bursula/install.sh
```
...
```dotenv
BURSULA_IMAGE=ghcr.io/bursula/bursula:0.3.13
...
```bash
docker run --rm ghcr.io/bursula/bursula:0.3.13
...
docker run --rm \
  ...
  ghcr.io/bursula/bursula:0.3.13
```
