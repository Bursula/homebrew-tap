...
```bash
docker run --rm ghcr.io/bursula/bursula:0.3.24 deployment export | tar -x && ./bursula/install.sh
```
...
```dotenv
BURSULA_IMAGE=ghcr.io/bursula/bursula:0.3.24
...
```bash
docker run --rm ghcr.io/bursula/bursula:0.3.24
...
docker run --rm \
  ...
  ghcr.io/bursula/bursula:0.3.24
```
