...
```bash
docker run --rm ghcr.io/bursula/bursula:0.3.19 deployment export | tar -x && ./bursula/install.sh
```
...
```dotenv
BURSULA_IMAGE=ghcr.io/bursula/bursula:0.3.19
...
```bash
docker run --rm ghcr.io/bursula/bursula:0.3.19
...
docker run --rm \
  ...
  ghcr.io/bursula/bursula:0.3.19
```
